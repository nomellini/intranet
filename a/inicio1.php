...
//		die($query_rsConsultores);
		...
		if ( $row_rsConsultores['qtde'] >= 4) {
			$msg = "N�o pode alterar para n�o dispon�vel";
		} else {
			if ($acao=="mudaestado") {
				$status = $status + 1;
				if ($status==10) {
		...
		document.formulario2.ausente_chamado.style.display = 'none'
                ...
                <td width="33%" align="center"> <p><a href="rnc/relatorios/index.php"><strong>Desktop
                  ...
                  <?
					...
					?>
                  <br>
                  <a href="espera/consultor.php" class="style1">Minhas liga��es</a>
                  <?
					 }
					  if ($DesktopSigame<>0) {
					?>
...
?>
              ...
              ?>
                ...
                <td width="33%" align="right"> <a href="/a/relatorios/relatbaseweb.php"><font size="1">Base
                  ...
                  <a href="javascript:online();">Quem est� On Line</a> <br>
                  <a href="/a/errosFujitsu.php"><font size="1">Erros vers�o FJ</font></a>
                  </td>
                </tr>
              </table></td>
          </tr>
			...
			while ( list($tmp1, $tmp) = each($chamadosemaberto) ) {
			   ...
			   if ($tmp["id_sistema"] == 1024) {
                 $cor_fundo = CORES_QUALIDADE;
   			   ...
   			   $rncTipo = '';
			   if ($tmp["rnc"]==1) {
			     $rncTipo = "<strong>N�o conformidade</strong>";
			     if (!$teste) {
    				 $cor_fundo = "#cccccc";
				 } else {
                   $cor_fundo = "#$teste";
...
?>
                    ...
                    <tr>
                      ...
                      <strong>Encaminhados n�o lidos</strong></span> </td>
                    </tr>
                    <?
	}
?>
...
?>
                    <tr>
                      ...
                      <strong><? if ($linha->quantidade!=0) {?><img src="figuras/idea01.gif" alt="Novidades" width="16" height="22" align="absmiddle"><? } ?>Contatos n�o lido </strong>(<?=$linha->quantidade?>) </span> </td>
                    </tr>
                    <?
	}
?>
...
?>
 ...
 <blockquote>Para ver a lista de chamados por prioridade ou por pasta, clique na prioridade ou na pasta desejada ao lado. Voc� pode criar quantas pastas desejar e incluir um chamado em qualquer pasta.</blockquote> </span>
                  </td>
              </tr>
...
contaencaminhados.innerHTML = '<?=$contaencaminhados?>';
 ...
 if( <?=$msgnova?> ) {
   msgnova.innerHTML = "<br>Existe <?=$msgnova?> chamado(s) com mensagens n�o lidas (<img src=figuras/idea01.gif  align=absmiddle>) [<a href='?naolido=1'>ver apenas n�o lidos</a>]";
 }

 if ( '<?=$naolido?>' == '1')
 {
   msgnova.innerHTML = "<br>Vendo apenas n�o lidos [<a href='?naolido=0'>ver todos</a>]";
 }
