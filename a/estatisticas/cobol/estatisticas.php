...
$SQLresumo = "select 
    s.sistema sistema,    	
    count(1) quantidade,
    sec_to_time(sum(time_to_sec(horae) - time_to_sec(co.horaa))) tempo
from
  chamado ch  
    inner join contato co on co.chamado_id = ch.id_chamado    
	  inner join status on status.id_status = ch.status
    inner join cliente cl on cl.id_cliente = ch.cliente_id
    inner join prioridade p on p.id_prioridade = ch.prioridade_id	
    inner join sistema s on s.id_sistema = ch.sistema_id
...
group by s.sistema"	;
	  ...
	  $result3 = mysql_query($SQLresumo) or die(mysql_error() . " - " . $sql)	  ;	  

	  
      while(  $linha3 = mysql_fetch_object($result3) ) 	{
		  

				$sistema = $linha3->sistema;
				$contatos = $linha3->quantidade;
				$tempo = $linha3->tempo;
...
<?
		...
		} // End while(  $linha2 = mysql_fetch_object($result) )
