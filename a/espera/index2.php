...
//	  $pesquisa = '';
    ...
    if ($acao == "pesquisacliente")  {
	  if ($nomesenha) {
         $sql = "select bloqueio, id_cliente, senha, cliente, uf ";
		 $sql .= " from ";
		 $sql .= " cliente ";
		 $sql .= "where ";
		 $sql .= "cliente like '%$nomesenha%' or senha = '$nomesenha' or id_cliente = '$nomesenha'";
	  } else if ($id_cliente) {
         $sql = "select bloqueio, id_cliente, senha, cliente, uf from cliente where id_cliente = '$id_cliente'";
	  }
	  $result = mysql_query($sql) or die($sql);
	  $qtde = mysql_affected_rows();
      $linha = mysql_fetch_object($result);
...
?>
  ...
  <tr>
    <td align="right"><input name="acao" type="hidden" id="acao" value="XXX">
      <input name="idligacao" type="hidden" id="idligacao">
Sad 2004 beta </td>
  </tr>
</table>
