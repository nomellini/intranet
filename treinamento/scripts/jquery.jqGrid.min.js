		...
		return this.each(function () {
			if (!this.grid) {
				var c = b.extend(true, {
					...
					onSortCol: null,
					ondblClickRow: null,
					onClickRow: null, /* Lucas: implementado para evento de click*/
...
//													asdadadasdaa; lucas
						...
						b.isFunction(this.p.onClickRow) && b(this).click(function (d) {
							V = d.target;
							R = b(V, a.rows).closest("tr.jqgrow");
							if (b(R).length === 0) return false;
							ja = R[0].rowIndex;
							ka = b.jgrid.getCellIndex(V);
							a.p.onClickRow.call(a, b(R).attr("id"), ja, ka, d);
							return false
						});
						...
						b(e.cDiv).append(Aa).append("<span class='ui-jqgrid-title" + (n == "rtl" ? "-rtl" : "") + "'>" + a.p.caption + "</span>").addClass("ui-jqgrid-titlebar ui-widget-header ui-corner-top ui-helper-clearfix "+a.p.handleName);
	...
	a.jgrid.extend({
		...
		viewGridRow: function (e, b) {
			...
			return this.each(function () {
				...
				function F(i, f, J, R) {
					for (var O, T, K, N = 0, V, Z, $ = [], Y = false, ca = "<td class='CaptionTD form-view-label ui-widget-content' width='" + b.labelswidth + "'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>", g = "", n = ["integer", "number", "currency"], x = 0, s = 0, G, B, C, L = 1; L <= R; L++) g += L == 1 ? ca : "<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>";
...
jQuery.fn.searchFilter = function (k, H) {
	function I(e, l, v) {
		...
		if (l != null) {
			...
			var f = jQuery.extend({}, jQuery.fn.searchFilter.defaults, v),
				n = -1,
				r = "";
				...
				if (c.grid) {
					...
					if (b("#" + f.themodal).html() != null) {
						...
						a.updateAfterCheck ? b(":input", "#" + d).click(function () {
							...
							a.ShrinkToFit === true && b(c).jqGrid("setGridWidth", c.grid.width - 0.0010, true);
							a.closeAfterSubmit && b.jgrid.hideModal("#" + f.themodal, {
								gb: "#gbox_" + e,
								jqm: a.jqModal,
								onClose: a.onClose
							});
	...
	a.jgrid.extend({
		sortableColumns: function (b) {
			return this.each(function () {
				...
				if (c.p.sortable.options) a.extend(h, c.p.sortable.options);
		...
		columnChooser: function (b) {
			...
			if (!a("#colchooser_" + h[0].p.id).length) {
				...
				b = a.extend({
					...
					apply_perm: function () {
						a("option", e).each(function () {
							this.selected ? h.jqGrid("showCol", g[this.value].name) : h.jqGrid("hideCol", g[this.value].name)
						});
						var f = [];
				...
				var g = h.jqGrid("getGridParam", "colModel"),
					i = h.jqGrid("getGridParam", "colNames"),
					l = {},
					m = [];
				e.empty();
				a.each(g, function (f) {
