...
GLOBAL $gSQLMaxRows,$gSQLBlockRows,$ADODB_ROUND;
	...
	while (!$rs->EOF) {
		$v0=($numoffset) ? $rs->fields[0] : reset($rs->fields);
		$v1=($numoffset) ? $rs->fields[1] : next($rs->fields);
		$v1 = urlencode($v1);
		$s .= "<TR onclick=\"fun_retorna_registro('" . $v0 . "','" . $v1 . "');\" valign=top class='texthome' onMouseOver=\"this.className='texto_tabela_hover'\" onMouseOut=\"this.className='texthome'\" >\n";
