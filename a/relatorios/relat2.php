...
mysql_select_db($database_sad, $sad);
$query_rsDiagnosticos = "SELECT * FROM diagnostico ORDER BY diagnostico ASC";
$rsDiagnosticos = mysql_query($query_rsDiagnosticos, $sad) or die(mysql_error());
$row_rsDiagnosticos = mysql_fetch_assoc($rsDiagnosticos);
$totalRows_rsDiagnosticos = mysql_num_rows($rsDiagnosticos);
?><?
...
?>
      ...
      <td><select name="id_diagnostico" class="bordaTexto" id="id_diagnostico">
        <option value="0" <?php if (!(strcmp(0, "$id_diagnostico"))) {echo "selected=\"selected\"";} ?>>Todos</option>
        <?php
do {  
?>
        <option value="<?php echo $row_rsDiagnosticos['id_diagnostico']?>"<?php if (!(strcmp($row_rsDiagnosticos['id_diagnostico'], "$id_diagnostico"))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsDiagnosticos['diagnostico']?></option>
        <?php
} while ($row_rsDiagnosticos = mysql_fetch_assoc($rsDiagnosticos));
  $rows = mysql_num_rows($rsDiagnosticos);
  if($rows > 0) {
      mysql_data_seek($rsDiagnosticos, 0);
	  $row_rsDiagnosticos = mysql_fetch_assoc($rsDiagnosticos);
  }
?>
      </select></td>
    ...
    <tr bgcolor="#FCE9BC">
      <td colspan="4">Limitar resultado em
        ...
        RNC 
        <input name="Ic_Atencao" type="checkbox" id="Ic_Atencao"  value="1" <? if($Ic_Atencao) {echo "checked";}?> >
        ATEN&Ccedil;&Atilde;O
        <input name="Ic_Qualidade" type="checkbox" id="Ic_Qualidade"  value="1" <? if($Ic_Qualidade) {echo "checked";}?> >
        Qualidade</td>
    </tr>
    <tr bgcolor="#FCE9BC">
...
<table width="98%" border="0" cellspacing="1" cellpadding="1" align="center" bgcolor="#000000">
  <tr bgcolor="#CCCCCC">
    <td width="9%" align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="1"><a href="javascript:document.form.ordem.value='ch.id_chamado'; document.form.submit();">Chamado</a></font></td>
...
mysql_free_result($rsDiagnosticos);
?>
