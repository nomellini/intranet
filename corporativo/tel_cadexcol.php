...
<? 
 ...
 if ($acao=="excluir") {
	...
	$sSQL = "DELETE from Telexcol WHERE id = $id;";
    mysql_query($sSQL);
	$acao = "excluído";
 } else {
 ...
 if (!$id) {
   ...
   if ($tipo == "excol") {
      ...
      $sSQL = "INSERT into Telexcol (ativo, nome, residencial, celular, homepage, email) VALUES (1, '$nome', '$residencial', '$celular', '$homepage', '$email');";
    };  
   ...
   if ($tipo == "excol") {
	  ...
	  $abr = strtoupper($abr); 
	  $divisao = strtoupper($divisao); 
      $sSQL = "UPDATE Telexcol SET nome='$nome', residencial='$residencial', celular='$celular',homepage='$homepage', email='$email' WHERE id = $id; ";
          mysql_query($sSQL);
   }
 
 }
}
header( "Location: telefones2.php#x$id") ;
 ...
 ?>
              ...
              <p><b> <? echo "$nome foi $acao" ?></b> com sucesso !<br>
                ...
                <a href="/corporativo/telefones2.php">Clique aqui para ver 
                os telefones cadastrados.</a></p>
