<?
 ...
 if(!mysql_connect(localhost, sad, data1371)) {
   ...
   $sSQL = "select id_cliente from cliente" ; 
   if (!$result1 = mysql_query($sSQL)) {
   echo "<br><br>Problema na Tabela cliente</b><br>";
   };   
   
   $sSQL = "select sistema from sistema" ; 
   if (!$result2 = mysql_query($sSQL)) {
   echo "<br><br>Problema na Tabela sistema</b><br>";
   };
?> 
...
function excluir() {
  if (confirm('Tem certeza que deseja excluir os dados ?')) {
	...
	document.form.submit();
			 ...
			 <p align="center"><span class="Titulo"><a href="/index.php">Home</a> : <?echo $action?> de Programas </span></p>
			 <form name="form" action="arquivo_cadastro.php" method="post" >
               <table width="95%" border="0" align="center">
                 <tr>
                   <td colspan="2"><div align="center"><a name="colaborador"></a>Utilize esta 
                     &aacute;rea para cadastrar um <b>Programa</b></div></td>
                 </tr>
                 <tr>
                   <td width="19%"><div align="right">Sistema : </div></td>
                   <td width="81%"><select name="sistema" id="sistema" onChange="document.form.sistema.value=this.value">
                       <option><? echo $sistema ?></option>
                       <? 

			   while ($linha = mysql_fetch_object($result2)){
			   $sistema=$linha->sistema;
			   print "<option value='".$sistema."'>".$sistema."</option>";
			   }
			   ?>
                     </select></td>
                 </tr>
                 <tr>
                   <td width="19%"><div align="right">Programa : </div></td>
                   <td width="81%"><input name="programa" type="text" id="programa" value="<? echo $programa ?>" size="30">                   </td>
                 </tr>
                 <tr>
                   <td width="19%"><div align="right">Descricao :</div></td>
                   <td width="81%"><input name="descricao" type="text" id="descricao" value="<? echo $descricao ?>" size="30" >                   </td>
                 </tr>
                 <tr>
                   <td><div align="right">Cliente :</div></td>
                   <td><select name="cliente" id="cliente" onChange="document.form.cliente.value=this.value" >
                       <option><? echo $cliente ?></option>
                       <? 
			   while ($linha = mysql_fetch_object($result1)){
			   $id_cliente=$linha->id_cliente;
			  print "<option value='".$id_cliente."'>".$id_cliente."</option>";
			   }
			   ?>
                     </select></td>
                 </tr>
                 <tr>
                   <td><div align="right">Chamado :</div></td>
                   <td><input name="chamado" type="text" id="chamado" value="<? echo $chamado ?>" size="30" > </td>
                 </tr>
                 <tr>
                   <td><div align="right">Observa&ccedil;&otilde;es :</div></td>
                   <td><input name="observacoes" type="text" id="observacoes" value="<? echo $observacoes ?>" size="30" >                   </td>
                 </tr>

                 <tr>
                   <td width="19%">&nbsp;</td>
                   <td width="81%"><input name="button" type=button onClick="javascript:confirma()" value="Alterar / Incluir">
