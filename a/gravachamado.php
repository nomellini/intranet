<?
	...
	$fl_PodeEncerrar = 1; //padrão = permite enderrar<br />
	$Ds_MotivoNaoEncerrar = "";
	if ($origem == 44) // Proposta envio
	{
		$fl_PodeEncerrar = 0; // Não permite
		$Ds_MotivoNaoEncerrar = "Proposta não aprovada ou não cancelada";		
	}
...
/*	foreach($id_restricao as $IdRestricao)
	...
	/**********************************************************************************
		Contato automático inserido pelo sistema (Público)
		
		07/10/2009 - Chamado 186.230
 		Criação do contato padrão para o chamado, pois todo chamado deverá ser OnLine !
		
		Não deve ser gerado com cliente DATAMACE, e se Sistema = (Qualidade = 1024)
	 **********************************************************************************/	
...
<p>O chamado foi considerado solucionado e encerrado no primeiro contato pela consultoria, caso você queira reabrir o chamado, acesse o Portal do cliente <a href=\"http://www.datamace.com.br/PortalCliente\">clicando aqui</a>
<br><br>
...
<br />
Caso você queira interagir no chamado ou&nbsp;saber&nbsp;sobre o andamento, acesse o Portal do cliente <a href=\"http://www.datamace.com.br/PortalCliente\">clicando aqui</a>
<br>
<br>
...
<p class=\"style1\">Consultoria Datamace</p>
	...
	/*
	 O Chamado já foi criado no início da página chamado.php
	 Ao criar o chamado, já foi definido o dono do chamado, as linhas
	...
	*/	
	...
	if ($prioridade == 7) {  // Aguardando outro chamado
		$Texto = "O chamado recém criado <b>$id_chamado</b> foi priorizado como 'Aguardando outro chamado' e está aguardando o chamado <b>$espera</b>, que se encontra em seu desktop.";
		...
		$subject = "Chamado com dependência";
	...
	/*
		Após atribuir a classe, gravo o chamado e o contato.
...
//	mysql_query($sql);
   ...
   if ( $action == "encaminhar" ) {  
	...
	$contato = "Descrição do chamado: $descricao <br><br>Contato: $historico";
	$mensagem = "$nome abriu um chamado :<br><a target=_blank href=$SITEROOT/a/historicochamado.php?id_chamado=$id_chamado>$id_chamado</a> que Contém uma um mais palavra chave cadastrada";	
	...
	*/
    ...
    /*
	  Envia uma carta padrão caso seja detectada necessidade de treinamento !!
	  
	  o arquivo com a carta padrão é o _email.txt
	  ...
	  Este código é repetido em gravachamado.php
	  
	  PS. Esta funcionalidade não mais envia email ao cliente, e sim para 
	  usuarios do sad cujo flag recebeemailtreinamento seja = 1, e normalmente
	  e o pessoal da área de treinamento.
	*/
