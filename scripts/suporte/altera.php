...
<title>suporte :: manutencao de titulo</title>
<!-- #EndEditable --> 
...
<div align="center"> 
 ...
 $sSQL = "Select * from titulo where id = $id";
 ...
 $result = mysql_query($sSQL); 
?> <br>
              Altera&ccedil;&atilde;o do t&iacute;tulo 
              <form method="post" action="altera02.php">
                <p>Sistema Operacional:<br>
                  <br>
                  <select name="so">
                    <?
     while($linha = mysql_fetch_object($result)) {
        ...
        $so = $linha->so;
    ?> 
                    <option value=<?=$idso;?> <? if($idso==$so_id) {print "selected";}?>   ><?=$so?></option>
                    <?
}
?> 
                  </select>
                ...
                <p>T&iacute;tulo <br>
                  <textarea name="titulo" cols="50" rows="2"><?=$titulo?></textarea>
                </p>
                <p>Obs.<br>
                  <textarea name="obs" cols="50" rows="5"><?=$obs?></textarea>
                </p>
                <p> 
                  <input type="checkbox" name="deleta" value="sim">
                  Apagar esse t&iacute;tulo. 
                  <input type="hidden" name="id" value="<?=$id?>">
                  <input type="submit" name="Submit" value="Enviar">
                </p>
                </form>
<a href="http://192.168.0.5/suporte/novo02.php?id_titulo=<?=$id?>">Inclu&iacute;r 
                uma nova Causa</a><br>
              <p>Alterar uma das seguintes Causas<br>
                <?
 mysql_connect(localhost, root, marcia);
 mysql_select_db(suporte);
 $sSQL = "SELECT * FROM descr where id_titulo=$id;";
 $result = mysql_query($sSQL);
 while($linha = mysql_fetch_object($result)) {
 $causa = $linha->causa;
 $id_causa = $linha->id;
?> -><a href="alteracausa.php?id_titulo=<?=$id?>&id_causa=<?=$id_causa?>"><?=$causa?></a><BR>
<? 
}
?>
