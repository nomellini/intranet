...
<?
 ...
 if ($acao == "excluir") {
    $sSQL = "DELETE from arquivos WHERE id = $id;";
    mysql_query($sSQL);
   $acao = "excluï¿½do";
 } else {
...
$chamado = strtoupper($chamado);
$observacoes = strtoupper($observacoes);

      $sSQL = "INSERT into arquivos (sistema, programa, descricao, cliente, chamado, observacoes) VALUES ('$sistema', '$programa', '$descricao', '$cliente','$chamado','$observacoes');";

   if(! mysql_query($sSQL)) {
     echo "<b>Registro Nï¿½O incluido</b><br>";
   }
 } else {
   ...
   if (!$alterar) {
     $divisao = $divisaooriginal;
      ...
      $sSQL = "UPDATE arquivos SET sistema='$sistema', programa='$programa', descricao='$descricao', cliente='$cliente', chamado='$chamado',observacoes='$observacoes' WHERE id = $id;";
      mysql_query($sSQL);
 ...
 ?>
              ...
              <p><b> <? echo "$nome foi $acao" ?></b> com sucesso !<br>
                <br>
                <br>
                Clique <a href="/arquivo.php">aqui</a> para
                cadastrar mais um, ou<br>
                Clique <a href="/ver_arquivo.php">aqui</a> para ver
                os programas cadastrados.</p>
