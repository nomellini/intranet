...
<div align="center"> 
 ...
 $sSQL = "Select distinct t.id as id_t, t.titulo, s.so from descr d, titulo t, so s where ( ( (titulo like '%$palavra%') or (solucao like '%$palavra%') or (causa like '%$palavra%') ) and (t.id_so = s.id) and ( d.id_titulo = t.id)) order by so, titulo;"; 
 ...
 if (!$palavra) {$palavra=" ";} 
?> 
              <form method="post" action="index.php" Style="Hidden">
                <p> 
                <blockquote> 
                  <p><a href="../a/"><img src="../a/SADLogo.gif" width="230" height="46" align="middle" border="0"></a><br>
                    <br>
                    <font size="3" color="#0000CC">Base de Solu&ccedil;&otilde;es</font><br>
                    Busca por palavra chave. </p>
                  <p><br>
                    <font size="1"> Para exibir todos, basta apertar o bot&atilde;o 
                    sem digitar nenhuma palavra.<br>
                    A busca por palavra chave procura pela palavra em todos os 
                    campo. ou seja, no titulo, na causa ou na solucao.<br>
                    <br>
                    </font> 
...
<? 
if ($palavra) {
  print "<p><blockquote>Resultado da busca pela palavra chave '<font color=#CC0000><b>$palavra</b></font>'<br>";
  print "$total ocorrências";
  print "</blockquote></p>";
}
?>
              <p>
<?
 ...
 if (!$result) {
   print "Nenhuma ocorrência foi encontrada";
 }
 while ($linha = mysql_fetch_object($result)) {
      ...
      if ($lso != $so) {
        print "<font size=2>";
        print "<br>Sistema Operacional : <b>$so</b><br>";
        $lso = $so;
        print "</font>";
      }
?> <font size="1"><?
  $titulo = $linha->titulo;
  $titulo = eregi_replace($palavra, "<b><font color=#ff0000>$palavra</font></b>", $titulo);
?>-<a href="detalhes.php?id=<?=$id?>&palavra=<?=$palavra?>"><?=$titulo?></a><br>
                </font> <?
}
?> </p>
              <p>&nbsp;</p>
