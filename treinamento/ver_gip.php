...
$sSQL = "SELECT C.nome, R.*, date_format(dataprova, '%d/%m/%Y') as data FROM respostas as R " .
		" left join cadastrotreinamento as C on C.rg = R.rg ";
$condicoes[] = (($mes) ? "month(dataprova) = $mes" : "");
$condicoes[] = (($ano) ? "year(dataprova) = $ano" : "");
$condicoes[] = (($flagTipo) ? "flagTipo = $flagTipo" : "");
$sSQL .= montaWhere($condicoes) . " order by $ordem, nome";
					...
					<tr>
						<td width="77%" align="left" valign="top" bgcolor="#FFFFFF"><!-- #BeginEditable "Centro" -->
							<form action="<?=$PAGINA ?>" method="post" name="form" id="form">
								...
								<p>&nbsp;</p>
								<p class="TituloTreino">Avaliações de eficácia / Provas</p>
								<table>
									<tr>
										<td><table width="732" border="1">
												<tr>
													<td colspan="2" class="thTreinamento">Procure pelo m&ecirc;s e ano</td>
												</tr>
												<tr>
													<td width="15%">Tipo :</td>
													<td width="85%"><?=fun_select($aProvaTipo,$flagTipo,'flagTipo',"",""," class='TXTLOAD'") ?></td>
												</tr>
												<tr>
													<td>Data :</td>
													<td>M&ecirc;s
				...
				?>
									...
									<tr>
										<td><table id="tabela" border="1" cellpadding="1" width="732" cellspacing="1">
												<thead>
													<tr>
														<td class="thTreinamento" width="41%"><a href="ver_gip.php?ordem=empnome<?=(($ordem=="empnome") ? " desc" : "") ?>">Nome da empresa</a></td>
														<td class="thTreinamento" width="41%"><a href="ver_gip.php?ordem=nome<?=(($ordem=="nome") ? " desc" : "") ?>">Nome</a></td>
														<td class="thTreinamento" width="10%"><a href="ver_gip.php?ordem=dataprova<?=(($ordem=="dataprova") ? " desc" : "") ?>">Data </a></td>
														<td class="thTreinamento" width="8%"><a href="ver_gip.php?ordem=cod_prova<?=(($ordem=="cod_prova") ? " desc" : "") ?>">Prova</a></td>
													</tr>
												</thead>
												<tbody>
													<? while ($linha = mysql_fetch_object($result)) { ?>
													<tr>
														<td><? echo "<a href=result_gip.php?id_resp=$linha->id&id=$linha->cod_prova>$linha->empnome</a>" ?></td>
														<td><? echo "<a href=result_gip.php?id_resp=$linha->id&id=$linha->cod_prova>$linha->nome</a>"  ?></td>
														<td><? echo "<a href=result_gip.php?id_resp=$linha->id&id=$linha->cod_prova>$linha->data</a>"?></td>
														<td><? echo "<a href=result_gip.php?id_resp=$linha->id&id=$linha->cod_prova>$linha->cod_prova</a>" ?></td>
													</tr>
													<? }; ?>
												</tbody>
											</table></td>
