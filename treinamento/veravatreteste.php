...
if ($id_opc == '2') {
	$pdf = new REL_PDF_QTD_TREINAMENTO($flagTipo, $evento, $ano, $mes, true);
	$pdf->gerar();
	...
	<table width="100%" border="0">
     ...
     document.write (diasemana[diaindex] + '<br>' + dia + ' de ' + mesescrito[mes] + ' de ' + ano);
				...
				<table width="100%" border="0" bordercolorlight="#FFCC30" bordercolordark="#FFCC30" class="bgTabela">
					<tr>
						<td width="77%" align="left" valign="top" bgcolor="#FFFFFF"><!-- #BeginEditable "Centro" -->
							...
							<p class="TituloTreino" align="center">Relatório de Quantidade de Avaliações do Treinamento</p>
								...
								<tr>
									<td><form action="<?=$PAGINA ?>" method="post" name="form" id="form">
											...
											<table width="100%" border="1" align="left">
												...
												<tr>
													<td width="20%">Tipo de Treinamento :</td>
													<td><?=fun_select($aTreinoTipo,$flagTipo,'flagTipo') ?></td>
												</tr>
												<tr>
													<td id="e_evento">Módulo :</td>
													<td><?=fun_select($ModulosID,$evento,'evento') ?></td>
												</tr>
												<tr>
													<td>Data :</td>
													<td>M&ecirc;s </font>
				...
				?>
								...
								<? } ?>
								<tr>
									<td><input name="BTNGravar" id="BTNGravar" type="button" value="Gerar" />&nbsp;<input name="BTNVoltar" id="BTNVoltar" type="button"  value="Voltar" voltarPara=<?=$linkPagina ?>/></td>
								</tr>
							</table>
							<!-- #EndEditable --></td>
...
include_once ('cabeca.inc.php');
 ...
 if ($mes) 
     $sSQL = "SELECT * FROM avaliatre where evento = '$evento' and month(data) = $mes and year(data) = $ano and flagTipo = '2' order by $ordem";
 else
     $sSQL = "SELECT * FROM avaliatre where flagTipo = '2' order by $ordem, evento";
	 $result = mysql_query($sSQL);  
	 
	 $sSQL = "select * from modulos "; 
...
function confirma(){ 
    if (!document.form.mes.value){ 
       alert("Deve preencher o campo mês") 
	...
	if (!document.form.evento.value){ 
       alert("Deve preencher o campo evento") 
	   document.form.evento.focus();
	   return false;     
	   }
      ...
      <td><table width="100%" border="0" bordercolorlight="#FFCC30" bordercolordark="#FFCC30">
          <tr>
            <td width="77%" align="left" valign="top" bgcolor="#FFFFFF">
              ...
              <span class="style8">Avalia&ccedil;&atilde;o Geral do Treinamento </span>            </strong> - <strong>M&oacute;dulos</strong> </div>
              <p>&nbsp;</p>
              <table width="80%" border="0" align="center">
                <tr>
                  <td><form method="post" action="veravatreteste.php" name="form" id="form" >
                    
                    <table width="100%" border="1" align="left">
                      <tr bgcolor="#D7FFFF">
                        <td align="center" valign="middle" bgcolor="#006699"><span class="style1 style11 style4"><strong>Procure pelo M&ecirc;s e Ano do evento </strong></span></td>
            </tr>
                      <tr>
