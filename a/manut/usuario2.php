	...
	$sql = "select u.nome, u.superior, s.nome as gestor
from usuario u left join usuario s on s.id_usuario = u.superior
where u.ativo = 0 and u.id_usuario = $id";	
	$query = mysql_query($sql);	
	if (!$linha = mysql_fetch_object($query))
	{
		header("Location: usuarios.php");
	}
	
	$GestorId = $linha->superior;
	$GestorNome = $linha->gestor;
	$UsuarioNome = $linha->nome;
	
	
	$sql = "select id_usuario, nome from usuario where ativo and gerencial or (id_usuario = $GestorId) order by nome";	
	$gestores = array();
	$query = mysql_query($sql) or die ("$sql - " . mysql_error());		
	$c=1;
	while ($linha = mysql_fetch_object($query))
	{
		$gestores[$c]["id"] = $linha->id_usuario;
		$gestores[$c++]["nome"] = $linha->nome;		
	}	
	
	
	$sql = "select count(1) qtde from chamado where status <> 1 and (consultor_id = $id or destinatario_id=$id)";
	$query = mysql_query($sql) or die ("$sql - " . mysql_error());		
	...
	if ($action == "alterar") {		
		...
		mysql_query("update chamado set consultor_id = $Id_Gestor where (status <> 1 and consultor_id = $id)");
		mysql_query("update chamado set destinatario_id = $Id_Gestor where (status <> 1 and destinatario_id = $id)");
        ...
        <table width="100%" border="0" bordercolorlight="#FFCC30" bordercolordark="#FFCC30" class="bgTabela">
          <tr>
            ...
            <form action="usuario2.php" method="post">
              ...
              &Uacute;suário <strong>
                <?=$UsuarioNome?>
                </strong>foi INATIVADO no SAD<br><br>            
                Clique abaixo para transferidos <?=$qtde?> chamados para o gestor:			
                <select name="Id_Gestor" >
                  <option selected value="0" >Selecione</option>
                  <? 
					foreach($gestores as $gestor) {                 
					$select = "";
					if ($GestorId == $gestor["id"])
					{
						$select = "selected = \"selected\"";
					}
                ?>
                  <option value="<?=$gestor["id"] ?>" <?=$select ?> >
                    <?=$gestor["nome"]?>
                  </option>
                  <? 
        	        } 
                ...
                <br>
                <br>
<input type="submit" name="Submit" id="button" value="Transferir" >

                <input name="id" type="hidden" id="id" value="<?=$id?>">
                <input name="action" type="hidden" id="action" value="alterar">
