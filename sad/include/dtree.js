...
// ----------------------------------------------------------------------------
	...
	// adiciona um node novo � disposi��o do node
...
// ----------------------------------------------------------------------------
	...
	// Imagens dePrealoads que s�o usadas na �rvore
	this.preloadIcons = function()
	{
		...
		this.arrIcons[4].src = this.imgFolder + 'spacer.gif';
		this.arrIcons[5] = new Image();
		this.arrIcons[5].src = this.imgFolder + 'spacer.gif';
	}

	// Fun��o Recursive de que cr�a a estrutura de �rvore
	this.addNode = function(pNode)
	{
		for (var n=0; n<this.arrNodes.length; n++)
		{
			if (this.arrNodes[n].pid == pNode)
			{
				...
				// se o node atual n�o for a raiz
				if (this.rootNode != cn.pid)
				{
					// escreve para fora a linha & �cones vazios
				...
				if (cn.url)
				{
					...
					// se a liga��o do hightlight for sobre
					if (this.clickSelect)
					{
						if (cn._hc)
						{
				...
				// escreve para fora o dobrador & pagina �cones
	...
	// verifica se uma identifica��o do node esta em um cookie
	...
	// Verifica��es se um node esta selecionado
	...
	// aberto ou pr�ximo todos os nodes
	...
	// retorna ids de n�s abertos como uma string
...
// As fun��es usadas pelo objeto do dTree mas n�o s�o realmente uma parte dela
// ------------------------------------------------------------------------------------------------
