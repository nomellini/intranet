...
<? 
 ...
 if ($acao == "excluir") {
    $sSQL = "DELETE from telUteis WHERE id = $id;";
    mysql_query($sSQL);
   $acao = "excluído";
 } else {
 ...
 if (!$id) {
   ...
   $acao = "incluído";
      ...
      $sSQL = "INSERT into telUteis (nome, telefone, fax, homepage, email) VALUES ('$nome', '$telefone', '$fax', '$homepage', '$email');"; 
   ...
   if (!$alterar) {
     $divisao = $divisaooriginal;
      ...
      $sSQL = "UPDATE telUteis SET nome='$nome', telefone='$telefone', fax='$fax', ";
      $sSQL = $sSQL . " homepage='$homepage', email='$email' WHERE id = $id;";
      mysql_query($sSQL);
 ...
 ?>
              ...
              <p><b> <? echo "$nome foi $acao" ?></b> com sucesso !<br>
                <br>
                <br>
                Clique <a href="/corporativo/alterartelutil.php">aqui</a> para 
