...
//	  $pesquisa = '';
    ...
    if ($acao == "pesquisacliente")  {
	  if ($nomesenha) {
         $sql = "select bloqueio, id_cliente, senha, cliente, uf, telefone ";
		 $sql .= " from ";
		 $sql .= " cliente ";
		 $sql .= "where ";
		 $sql .= "cliente like '%$nomesenha%' or senha = '$nomesenha' or id_cliente = '$nomesenha'";
	  } else if ($id_cliente) {
         $sql = "select bloqueio, id_cliente, senha, cliente, uf, telefone from cliente where id_cliente = '$id_cliente'";
	  }
	  $sql_1 = $sql;
	  $result = mysql_query($sql) or die($sql);
	  $qtde = mysql_affected_rows();
	  ...
	  if ($qtde == 1) {
		...
		$ddd = "";		
		if ($posAbre > 0) {	
			$ddd = substr($telefone, $pos, 2);    
		}				
		$cliente .= " Fone: ($ddd) $telefone";		
...
//  $sql = "select fl_posvenda from clienteplus where id_cliente = " . QuotedStr($id_cliente);
  ...
  $sql = "select qtde_aguarde,  id, cliente.uf, cliente.cliente, sec_to_time(  time_to_sec(curtime()) - time_to_sec(hora_inicio)  ) as espera, ";
  ...
  while ($linha = mysql_fetch_object($result)) {
	...
	if ($uf != 'SP') {
	  $cliente = "<b>$cliente - Fora do Estado ($uf)</b>";
	}
?>
