        ...
        <table width="100%" border="0" bordercolorlight="#FFCC30" bordercolordark="#FFCC30" class="bgTabela">
          <tr>
            <td width="77%" align="left" valign="top" bgcolor="#FFFFFF"><!-- #BeginEditable "Centro" --> 
              <p>&nbsp;</p>
              <p>&nbsp;</p>
              <p> TOP Secret - MUITO CUIDADO. Um Clique e o Registro &eacute; 
                Apagado do Banco de dados.</p>
              <p>
<? 
 mysql_connect(localhost, root, marcia);
 mysql_select_db(datamace);
 $sql = "select * from TelCol order by nome;";
...
?></p>
  ...
  while ($linha = mysql_fetch_object($result) ) {
    $id = $linha->id;
    $nome = "<a href=alterartelcol.php?id=$id>" . $linha->nome . "</a>";
    $estado = $linha->ativo;
    if ($estado) {
      $acao = "<a href=apaga.php?id=$id&acao=desativar>desativar</a>";
    } else {
      $acao = "<a href=apaga.php?id=$id&acao=ativar>ativar</a>";
      $nome = "<b><i>$nome</i></b>";
    }
   ...
   echo "<td>$nome</td>";
   echo "<td>$acao</td>";
   echo "<td><a href=apaga.php?id=$id&acao=apagar>apagar</a></td></tr>";
