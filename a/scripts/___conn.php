...
Consultoria Datamace";
	...
	function conn_projetos_EmailInclusao($id_chamado_filho, $id_chamado_pai, $ok)
	{
		...
		$msg = "O chamado $id_chamado_filho foi inclu�do no projeto $id_chamado_pai por <b>$nome</b><br><br>";
		$msg .= conn_projetos_obterEstatisticas($id_chamado_pai);
		...
		if (count($emails) == 0)
			$emails = split(";", $e);
		foreach( $emails as $email ){
			mail2($email, "Projeto $id_chamado_pai - Chamado inclu�do: $id_chamado_filho", $msg, "Projetos");
		}
	}
	...
	function conn_projetos_EmailExclusao($id_chamado_filho, $id_chamado_pai, $ok)
	{
		...
		$nome = peganomeusuario($ok);
		$msg = "O chamado $id_chamado_filho foi exclu�do do projeto $id_chamado_pai por <b>$nome</b><br><br>";
		$msg .= conn_projetos_obterEstatisticas($id_chamado_pai);
		...
		if (count($emails) == 0)
			$emails = split(";", $e);
		foreach( $emails as $email ){
			mail2($email, "Projeto $id_chamado_pai - Chamado exclu�do: $id_chamado_filho", $msg, "Projetos");
		}
...
function pegaMarketing($id) {
     $sql = "select count(1) as qtde from usuario where (id_usuario = $id) and (area = 7 or area = 4 or id_usuario=12);";
     $result = mysql_query($sql);
     $linha=mysql_fetch_object($result);
     if ($linha) {
         return $linha->qtde;
...
function pegaTreinados($cliente) {
	...
	$sql = "SELECT sistema, A.nome as nome, B.conceito_id as conceito_id, B.conceito as conceito from sad.treinados as A".
...
function loga_email_EncerraChamadoSigame($id_usuario, $id_chamado)
{
	...
	$mensagem = "$nome encerrou um chamado:<br><a target=_blank href=$SITEROOT/a/historicochamado.php?id_chamado=$id_chamado>$id_chamado</a> - $descricao<br><br>Cont�m uma um mais palavra chave cadastrada";
...
function loga_email_NovoContatoSigame($id_usuario, $id_chamado)
{
	...
	$mensagem = "$nome inseriu um contato no chamado:<br><a target=_blank href=$SITEROOT/a/historicochamado.php?id_chamado=$id_chamado>$id_chamado</a> - $descricao<br><br>Cont�m uma um mais palavra chave cadastrada";
	loga_email_PalavraChave($id_usuario, $id_chamado, $mensagem, $contato);
}
