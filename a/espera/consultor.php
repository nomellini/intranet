<?	
	...
	*/
    ...
    if ($acao == 'devolver') {
     if ($id_ligacao) {
       ...
       $sql = "update usuario set estado = 1, estado_hora = '$agoraTimeStamp', sat_idcliente = '' where id_usuario = $ok";
	   mysql_query($sql);
   ...
   if ($acao=='atender') {
       if ($id_ligacao) {
		   ...
		   $senha_cliente = $id_cliente;
           $sql = "select id_cliente from cliente where senha = '$senha_cliente'";
		   $result = mysql_query($sql);
		   $linha = mysql_fetch_object($result);
		   $id_cliente_usuario = $linha->id_cliente;
		   
		   
		   $sql = "update usuario set estado = 4, estado_hora = '$agoraTimeStamp', sat_idcliente = '$id_cliente_usuario' where id_usuario = $ok";
		   mysql_query($sql);
   ...
   $sql .= "  sistema.sistema as produto, linha, qtde_aguarde, cliente.uf, cliente.telefone ";
   $sql .= "from  ";
      ...
      <tr>
        ...
        <td width="84%" ><span class="style14"><font color="#003333">
          <?
		    echo "$cliente [$senha]";
			if ($estado != 'SP') {
			 echo "<BR><H2>CLIENTE DE FORA DO ESTADO <BR>$estado - $telefone!</H2> ";
			}
		  ?>
        </font></span></td>
      </tr>
...
<?
  $sql = "select cliente.cliente, cliente.senha, sec_to_time(  time_to_sec(curtime()) - time_to_sec(hora_inicio)  ) as espera, ";
...
?>
  ...
  <tr>
    <td align="right"><form name="form1" method="post" action="">
      ...
      <input name="id_cliente" type="hidden"  value="<?=$senha?>">
