...
$sql .= "ch.categoria_id as categoria, ";
//$sql .= "motivo.motivo, ";
$sql .= "ch.motivo_id as motivo, ";
//$sql .= "id_cliente, ";
$sql .= "id_chamado as chamado_id, ";
$sql .= "left(ch.descricao, 60) as descr, ";
$sql .= "ch.descricao as descricaoc, ";
$sql .= "cliente_id as cliente ";
$sql .= "FROM ";
if ($palavra) {
  $sql .= "contato, ";
}  
...
$sql .= "WHERE ( ";
if ($palavra) {
  $sql .= "( contato.chamado_id = ch.id_chamado ) AND ";
}  
$sql .= " ( ch.dataa >= '$datai' ) ";
...
if ($categoria) {
  $sql .= "AND ( ch.categoria_id = $categoria ) ";
}
...
$sql .= "AND ( sistema.id_sistema = ch.sistema_id) ";
...
function pegaConsultores($atendimento) {
	 $sql = "SELECT DISTINCT usuario.id_usuario, ";
	 $sql .= "usuario.nome from usuario, ";
	 $sql .= "chamado ";
	 $sql .= "where ( ";
	 $sql .= " (chamado.consultor_id=usuario.id_usuario) ";
	 $sql .= "AND (usuario.atendimento = $atendimento) )";
	 $sql .= "order by nome;";
...
$df = "$quando[2]-$quando[1]-$quando[0]";
    ...
    $sql = "select nome, count(*) as contatos from contato, usuario where (usuario.id_usuario=contato.consultor_id) ";
    if($o) {
...
$sql = "SELECT id, sistema.sistema, usuario.nome, diagnostico.diagnostico, baseweb.id_chamado as chamado, baseweb.programa, baseweb.data, ";
...
function statContatosPorConsultor2($c, $o, $di, $df) 
