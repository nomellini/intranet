...
// Linha abaixo já corrige aspas e barra 
...
if ( $_POST["action"] == "cancelar") {
	...
	die(""); // coloquie o die aqui pq o PHP não interrompeu o programa neste ponto
}
...
if ($action=="contato") {
  ...
  header("Location: chamado.php?id_cliente=$cliente_id&id_ligacao=$id_ligacao&horae=$horae");
	...
	{		
		$historico = "$historico<br><b>-></b><font color=#666666>Versão mudou de \"$objChamado->Ds_Versao\" para \"<b>$Ds_Versao</b>\".</font>";  	  	  				
		$objChamado->Ds_Versao = $Ds_Versao;		
	}
	...
	*/
	...
	// Não sou dono E a prioridade era 4 (aguardando cliente) E mudou a prioridade	
...
//  */
	...
	if ($origem == 44)
	{
		$objChamado->fl_PodeEncerrar = 0; // permite não
		$objChamado->Ds_MotivoNaoEncerrar = "Proposta não aprovada ou não cancelada";
	}	
...
//		if ($ok == 245)	{				die ( "<h1> $objChamado->status  - $pendente<h1>");		}	
	...
	/*
		$publicar - Checkbox na tela, indica de publico ou não.
	*/
    if($publicar) {
...
//    require("scripts/email.php");
    ...
    /*
	  Envia uma carta padrão ao cliente indicado
	...
	*/
	...
	if ($action == "encerrar") {	
		...
		/* 
			...
			Mas quem é da administração não deve enviar email
		*/			
		if ($atendimento==1) {
		...
		{
			/*
				A Não ser que explicitamente pediu para publicar.
		...
		/*
		   Devo mandar email se o existem chamados que estão
		   aguardando a conclusão deste aqui
		*/
	...
	if(($base) && ($base_desc != '')) {
		...
		if (!$base_cliente) {
			$base_cliente = "0"; // Base cliente = documentação web, vai para o boletim.
		} else {
