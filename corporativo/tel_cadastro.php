...
<? 
 ...
 if ($acao=="excluir") {
	...
	$sSQL = "DELETE from TelCol WHERE id = $id;";
    mysql_query($sSQL);
	$acao = "excluído";
 } else {
 ...
 if (!$id) {
   ...
   if ($tipo == "col") {
      ...
      $sSQL = "INSERT into TelCol (nome, divisao, residencial, celular, homepage, email, ramal, nasc, abr, ativo) VALUES ( '$nome', '$divisao', '$residencial', '$celular', '$homepage', '$email', '$ramal', '$data', '$nomecurto', 1)";
    } else
   {
      $nome = strtoupper($nome); 
      $sSQL = "INSERT into telUteis (nome, descricao, telefone, fax, contato, homepage, email) VALUES ( '$nome', '$descricao', '$telefone', '$fax', '$contato', '$homepage', '$email', '$endereco','$horario');"; 
   } ;  

   if(! mysql_query($sSQL)) {
     echo "<b>Registro NÃO incluido</b><br>";
   }
 } else {
   ...
   if ($tipo == "col") {
	  ...
	  $abr = strtoupper($abr); 
	  $divisao = strtoupper($divisao); 
      $sSQL = "UPDATE TelCol SET nome='$nome', divisao='$divisao', residencial='$residencial', celular='$celular',homepage='$homepage', email='$email', ramal='$ramal', nasc='$data', abr='$nomecurto' WHERE id = $id; ";
          mysql_query($sSQL);
   }
...
header( "Location: telefones2.php#$id") ;
...
?>
              ...
              <p><b> <? echo "$nome foi $acao" ?></b> com sucesso !<br>
                <br>
                <br>
                Clique <a href="../corporativo/alterartelcol.php">aqui</a> para 
                cadastrar mais um, ou<br>
                Clique <a href="../corporativo/telefones2.php">aqui</a> para ver 
                os telefones cadastrados.</p>
