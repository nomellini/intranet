...
<title>Detalhamento :: Suporte</title>
<!-- #EndEditable --> 
...
<div align="center"> 
 ...
 $sSQL = "update titulo set visitas = visitas + 1 where id = $id;";
 mysql_query($sSQL); 
 
 if (!$palavra) {$palavra=" ";}

 $sSQL = "SELECT t.*, s.so FROM titulo t, so s where ( (t.id=$id) and (s.id=t.id_so) );"; 
 ...
 $so     = $linha->so;
 $obs	 = $linha->obs;
 $obs = eregi_replace($palavra, "<b><font color=#ff0000>$palavra</font></b>", $obs);
 $visitas = $linha->visitas;
?></p>
              <p> 
              <blockquote> 
                <p> <font color="#000099" size="3"><?=$titulo?></font><br>
                  Sistema operacional :<b> <?=$so?></b><br>
                  <br>
                  Obs. :<i> <?=$obs?></i> <br>
                  <br>
                  <?
 mysql_connect(localhost, root, marcia);
 mysql_select_db(suporte);
 $sSQL = "SELECT * FROM descr where id_titulo=$id order by id ;";
 $result = mysql_query($sSQL);
 while($linha = mysql_fetch_object($result)) {
 $causa = $linha->causa;
 $causa = eregi_replace($palavra, "<b><font color=#ff0000>$palavra</font></b>", $causa);
 $solucao = $linha->solucao;
 $solucao = eregi_replace($palavra, "<b><font color=#ff0000>$palavra</font></b>", $solucao);
?> <font color="#000000"><b>Causa</b><br>
                  <?=$causa?><br>
                  <br>
                  <b>Solu&ccedil;&atilde;o</b><br>
                  <?=$solucao?><br>
                  </font></p>
                <hr size="1" noshade>
                <br>
...
?> 
<br>
                Esse t&oacute;pico j&aacute; foi lido : <?=$visitas?> vez(es)<br>
<a href="javascript:history.go(-1);">Voltar &agrave; p&aacute;gina anterior</a></blockquote>
              <!-- #EndEditable --></td>
