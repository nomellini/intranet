...
<title>suporte :: Estatísticas</title>
<!-- #EndEditable --> 
...
<?
 ...
 $sSQL = "select sum(visitas) as soma, id_so, so  from titulo, so where (so.id=titulo.id_so) group by id_so;";
 ...
 while ( $linha1 = mysql_fetch_object($result1) ) {
   ...
   print "<br><b>$so</b><br><br>";

   $sSQL = "select visitas, titulo, so from titulo, so where (id_so=$id_so) and (titulo.id_so = so.id) order by visitas desc;";
   $result = mysql_query($sSQL);    
   while($linha = mysql_fetch_object($result)) {
     $titulo = $linha->titulo; 
     $visitas = $linha->visitas;
     if ($soma!=0) {
       $porc    = ((int)( ($visitas / $soma * 100) *100))/100; 
     } else $porc = 0;
     print "$porc % - $visitas - $titulo<br>";
  }
}
