...
if ($id_opc == '2') {
		...
		$result			= mysql_query("select * from modulos where id = $mod_id"); 
		$prova_ok		= mysql_num_rows($result);
		
		$mod_descricao	= strtoupper($mod_descricao);
		
		if ($prova_ok > 0){
			$msgok = 'U';
			$sSQL = "UPDATE modulos set descricao='$mod_descricao', cod_sistema='$mod_cod_sistema', cod_sistema_sad = '$mod_cod_sistema_sad' where id = $mod_id";
		}else{
			$msgok = 'I';
			$sSQL = "INSERT into modulos  (id, descricao, cod_sistema, cod_sistema_sad) VALUES ('$mod_id', '$mod_descricao', '$mod_cod_sistema', '$mod_cod_sistema_sad')";
		}
		mysql_query($sSQL);
	...
	$result		= mysql_query("delete from modulos where id = $mod_id");
	$msgok		= 'E';
	$mod_id		= '';
}elseif ($id_opc == '99') {
	$mod_id		= '';
...
if ($id_opc == '1') {
	...
	if (mysql_num_rows($result) > 0){
		...
		$mod_id				= $linha->id;   
		$mod_descricao		= $linha->descricao;
		$mod_cod_sistema	= $linha->cod_sistema; 
		$mod_cod_sistema_sad = $linha->cod_sistema_sad;
	}else{
		$mod_id			= '';
	}
}
...
if (!$mod_id){
	$result				= mysql_query("select (max(id) + 1) as novo from modulos");
	$linha				= mysql_fetch_object($result);
	$mod_id				= $linha->novo;
	$mod_descricao		= '';
	$mod_cod_sistema	= '';
...
<script>
	...
	var a_fields = {
		'mod_id':{'r':true,'f':'integer'},
		'mod_descricao':{'r':true},
		'mod_cod_sistema':{'r':true}
	},
	o_config = {
					...
					<tr>
						<td width="77%" align="left" valign="top" bgcolor="#FFFFFF"><!-- #BeginEditable "Centro" -->
							<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" name="form" id="form" >
								...
								<p>&nbsp;</p>
								<p class="TituloTreino" align="center">Cadastro de Módulos</p>
								<table width="100%" border="0" align="center">
									<tr>
										<td valign="bottom" id="e_mod_id">C&oacute;digo   :</td>
										<td><input name="mod_id" type="text" id="mod_id" value="<?=$mod_id ?>" size="50" maxlength="10" class="TXTINT TXTLOAD" help="modulos" helpid_opc="1" pri_focu="S"></td>
									</tr>
									<tr>
										<td width="158" valign="bottom" id="e_mod_descricao">Descri&ccedil;&atilde;o do M&oacute;dulo  :</td>
										<td width="546"><input name="mod_descricao" type="text" id="mod_descricao" value="<?=$mod_descricao ?>" size="50" maxlength="200"/></td>
									</tr>
									<tr>
										<td valign="bottom" id="e_mod_cod_sistema">Sistema  Treinamento:</td>
										<td><?=fun_select($SistemasID,$mod_cod_sistema,'mod_cod_sistema') ?></td>
									</tr>
									<tr>
										<td valign="bottom">Sistema SAD:</td>
										<td><?=$db->comboBox("select id_sistema, sistema from sad.sistema order by id_sistema",$mod_cod_sistema_sad,'mod_cod_sistema_sad','','S') ?></td>
									</tr>
									<tr>
										<td valign="bottom">&nbsp;</td>
										<td>&nbsp;</td>
